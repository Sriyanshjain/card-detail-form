...
class Creditcard extends Component {
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            ...
            if (setupTime === null) //this only works first time when there is no value in session storage
            {
                sessionStorage.setItem('setupTime', now)
            }

            else {    //comparing time passed with the specified time of session

                if (now - setupTime > hours * 60 * 60 * 1000) //session time exceeds 30 min
                {
    ...
    validateInput = (e) => {
        ...
        //works when function is invoked by cardNumber input
            ...
            ele.value = ele.value.replace(/[A-Za-z}"`~_=.\->\]|<?+*/,;\[:{\\!@#\/'$%^&*()]/g, "");
    ...
    handleInputChange = (e) => {
        ...
        if (id === 'cardHolder') {
            var ele = document.getElementById(id);
            //if user enters any invalid characters it gets replaced 
            ele.value = ele.value.replace(/[}"`~_=.\->\]|<?+*/,\d;\[:{\\!@#\/'$%^&*()]/g, "");
            this.setState({ [name]: ele.value });
        }
        else
            this.setState({ [name]: value });
        ...
        return (
            ...
            <div>
                ...
                <div className="credit-card ">
                    <Cards locale={{ valid: "Expires" }} placeholders={{ name: 'FULL NAME' }}
                        cvc={this.state.cvc}
                ...
                <div className="card">
                    ...
                    <form className="payment-form">
                        <div className="form-group">
                            ...
                            <input type="text" onChange={this.validateInput} value={this.state.number} onKeyDown={this.removeSpecial} onPaste={e => e.preventDefault()} onKeyPress={this.addSpace}
                                onFocus={this.handleInputFocus} name="number" maxLength="19" id="cardNumber" className="form-control form-control-lg" />
                        </div>
                        <div className="form-group">
                            <label htmlFor="cardHolder" className="card-label" >Card holder</label>
                            <input type="text" name="name" spellCheck="false" value={this.state.name} maxLength="20" autoComplete="off" onPaste={e => e.preventDefault()} onChange={this.handleInputChange} onFocus={this.handleInputFocus} id="cardHolder" className="form-control form-control-lg" />
                        </div>
                        <div className="date-cvv-box">
                            <div className="expiry-class">
                                <div className="form-group card-month ">
                                    <label htmlFor="cardMonth" className="card-label" >Expiration Date</label>

                                    <select id="cardMonth" data-ref="cardDate" value={this.state.expiry} name="expiry" onChange={this.handleInputChange} onFocus={this.handleInputFocus} className="form-control form-control-lg">
                                        <option value="" defaultChecked="true" >Month</option>
                                ...
                                <div className="form-group card-year" >
                                    ...
                                    <select id="cardYear" data-ref="cardDate" value={this.state.expiryyear} name="expiryyear" onChange={this.handleInputChange} onFocus={this.handleInputFocus} className="form-control form-control-lg">
                                        <option value="" defaultChecked="true">Year</option>
                            ...
                            <div className="cvv-class form-group">
                                ...
                                <label htmlFor="cvv" className="card-label cvv-label" >CVV</label>
                                <input type="number" onChange={this.validateInput} onKeyDown={this.removeSpecial} onPaste={e => e.preventDefault()} onFocus={this.handleInputFocus}
                                    name="cvc" id="cvv" value={this.state.cvc} className="form-control form-control-lg " maxLength="4" />
